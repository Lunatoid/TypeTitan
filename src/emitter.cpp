...
// All types that have been scanned
// key: qualified name
static std::set<std::string> all_types;

// First 5 values are hardcoded:
// 0 : invalid typeid
// 1 : T*
// 2 : T&
// 3 : T&&
// 4 : T [size]
static int last_type_id = 5;
...
int get_type_count() {
    return last_type_id + (int)primitives_to_emit.size();
}
    ...
    if (!data.methods.empty()) {
        ...
        for (int i = 0; i < data.methods.size(); i++) {
            ...
            output <<
                ...
                "            methods[" << i << "].type_id = " << last_type_id++ << ";\n"
            ...
            output <<
                ...
                "            type_table[methods[" << i << "].type_id] = &methods[" << i << "];\n\n";
